/**
 * HashCashTable — Durable Object with Hibernation WebSocket API
 *
 * One instance per table (e.g. "hashcash-1", "hcpriv-ABC123").
 * ALL game logic lives here — bets, dice, resolution, state.
 * No crypto, no wallets, no on-chain — pure game server.
 * Players auth with username + HMAC session token.
 *
 * ── REST endpoints (called by Worker only) ──
 *   GET  /info                                → { count, players }
 *   POST /lockin    { username }               → return score, mark done
 *
...
export class HashCashTable {
  ...
  async fetch(request) {
    ...
    // ── Lock In (POST) — return player score and remove from table ──
    if (url.pathname === '/lockin' && request.method === 'POST') {
      const body = await request.json();
      return await this._handleLockin(body);
    }
    ...
    const playerId = username; // username IS the player ID in hashcash
    const name = username || 'Unknown';
    const chadId = Math.floor(Math.random() * 20) + 1; // random chad avatar

    if (!username || !sessionToken) {
      return new Response('Missing username or sessionToken', { status: 400 });
    }
  ...
  async webSocketMessage(ws, msg) {
    ...
    switch (data.type) {
      ...
      // ── Auth: link WS connection to player ──
      case 'auth': {
        const username = data.username;
        const token = data.sessionToken;
        const nonce = username; // username is the key in hashcash
        if (!username || !token) {
          ws.send(JSON.stringify({ type: 'error', message: 'Missing username or sessionToken' }));
          break;
        }
      ...
      case 'bet': {
        ...
        if (!pData) break;
        if (pData.spectator) {
          ws.send(JSON.stringify({ type: 'bet-rejected', reason: 'Spectating — cannot bet' }));
          break;
        }
        const game = await this._getGame();
      ...
      case 'roll': {
        ...
        {
          const rollPd = await this.state.storage.get(`player:${attachment.nonce}`);
          if (rollPd && rollPd.spectator) {
            ws.send(JSON.stringify({ type: 'error', message: 'Spectating — cannot roll' }));
            break;
          }
        }
  ...
  async _handleLockin(body) {
    const { username } = body;
    if (!username) {
      return jsonResp({ error: 'Missing username' }, 400);
    }

    const nonce = username;
  ...
  async alarm() {
    ...
    // ── Disconnected player cleanup: remove after 10 min grace period ──
  ...
  async _advanceShooter(currentShooterId) {
    ...
    // Build ordered list of active (non-spectator, non-disconnected) playerIds
    const playerIds = [];
  ...
  async _getPlayerCount() {
    ...
    let count = 0;
    for (const [, pd] of all) {
      if (!pd._disconnectedAt && !pd.spectator) count++;
    }
    return count;
  }
