...
const { MARKET: MARKET_ADDRESS, MEMBERS_ONLY: CONTRACT_ADDRESS, ITEMS: ITEMS_ADDRESS } = require('./addresses');
...
async function main() {
  ...
  console.log("Approving MembersOnly (ERC-721)...");
  const tx1 = await market.setApprovedContract(CONTRACT_ADDRESS, true);
  await tx1.wait();
  console.log("  Done:", tx1.hash);

  console.log("Approving MembersOnlyItems (ERC-1155)...");
